...
def fetch_all_tables():
    try:
        ...
        drilling_parameter_data = [object_to_dict(drilling_param) for drilling_param in DrillingParameter.query.all()]
        afe_data = [object_to_dict(afe) for afe in AFE.query.all()]
        personnel_data = [object_to_dict(personnel) for personnel in PersonnelInCharge.query.all()]
        summary_data = [object_to_dict(summary) for summary in Summary.query.all()]
        time_breakdown_data = [object_to_dict(time_breakdown) for time_breakdown in TimeBreakdown.query.all()]
...
def fetch_report_by_id(report_id):
    try:
        ...
        report_data = {
            ...
            "drilling_parameters": object_to_dict(drilling_parameter_data) if drilling_parameter_data else None,
            "afe": object_to_dict(afe_data) if afe_data else None,
            "personnel_in_charge": object_to_dict(personnel_data) if personnel_data else None,
            "summary": object_to_dict(summary_data) if summary_data else None,
            "time_breakdown": [object_to_dict(tb) for tb in time_breakdown_data] if time_breakdown_data else None,
        }
